...
//Feinde platzieren
function placeEnemy() {
  //Funktion um Feinde zu zeichnen
  for (i = 0; i < enemyPosition.length; i++) {
    //for-Schleife nach Anzahl der Feinde mit Länge des Arrays festlegen
    enemyPosition[i] += enemyMovement[i] * enemyPace; //Geschwindigkeit der Feinde einstellen
    if (enemyPosition[i] > enemyPosMaxx || enemyPosition[i] < enemyPosMinx) {
      //if-Abfrage zum Standort der Feinde
...
function checkEnemyCollison() {
  //Funktion zur Abfrage ob Truck mit Feind Kontakt hatte
  for (i = 0; i < enemyPosition.length; i++) {
    //for-Schleife nach Anzahl der Feinde mit Länge des Arrays festlegen
    var enemyy = firstEnemy - i * distEnemy; //Aktuelle Feindposition in Variable speichern
    if (
      truckx < enemyPosition[i] + objectWidth && //Abfrage auf Kollisionen bei leichter Berührung
...
function enemyCollision() {
  //Funktion zur Anzeige Gameover nach einer Kollision
...
function targetCollision() {
  //Funktion nach erreichen des Ziels
  ...
  if (truckx == targetx && trucky == targety) {
    //Abfrage auf Zielkollision
...
$(document).ready(function() {
  ...
  function moveDown() {
    //Funktionsaufruf Bewegung nach unten
    trucky += stepSize; //Truck um stepSize nach unten bewegen
    if (trucky >= vertMaxy) {
      //Abfrage ob playgroundgrenze nach unten erreicht
  ...
  function moveLeft() {
    //Funktionsaufruf Bewegung nach links
    truckx -= stepSize; //Truck um stepSize nach links bewegen
    if (truckx <= horMinx) {
      //Abfrage ob playgroundgrenze nach links erreicht
      truckx = truckMinx; //Maximal mögliche Bewegung nach links
    }
  }

  function moveRight() {
    //Funktionsaufruf Bewegung nach rechts
    truckx += stepSize; //Truck um stepSize nach rechts bewegen
    if (truckx >= horMaxx) {
      //Abfrage ob playgroundgrenze nach rechts erreicht
  ...
  $(document).bind("keydown", function(evt) {
    ...
    switch (
      evt.keyCode //Switch-Case Abfrage auf Tastendruck
    ) {
